    ...
    for col_name in nominal_cols:
        if col_name in header:
            ...
            for row in range(2, ws.max_row + 1):
                ...
                cell.number_format = '#,##0'  # Format angka ribuan

def point_1():
    print("=== PLATINUM POINT 1 ===")
    ...
    for pma, group in df_point1.groupby("PMA"):
        ...
        if os.path.exists(file_path):
            ...
            for row_idx, row in enumerate(group.itertuples(index=False), start=2):
        ...
        #--- save region ---
        region_file = os.path.join(output_folder, "region.xlsx")
        sheet_name = "PLATINUM"

        if os.path.exists(region_file):
            wb = load_workbook(region_file)
            if sheet_name in wb.sheetnames:
                del wb[sheet_name]
            ws = wb.create_sheet(title=sheet_name)
            # Tulis header
            for col_idx, col_name in enumerate(df_point1.columns, 1):
                ws.cell(row=1, column=col_idx, value=col_name)
            # Tulis data
            for row_idx, row in enumerate(df_point1.itertuples(index=False), start=2):
                for col_idx, value in enumerate(row, 1):
                    ws.cell(row=row_idx, column=col_idx, value=value)
        else:
            df_point1.to_excel(region_file, sheet_name=sheet_name, index=False)
            wb = load_workbook(region_file)
            ws = wb[sheet_name]

        # Auto adjust lebar kolom dan format nominal
        auto_adjust_column_width(ws)
        format_nominal_columns(ws, nominal_cols=["TARGET", "VALUE", "INSENTIF"])
        wb.save(region_file)
    return df_point1
